...
const Account = () => {
  ...
  const [activeComponent, setActiveComponent] = useState(<AccountInfo user={user}  token={token}/>);
      ...
      <div className="container">
        ...
        >
          <div className="col-5 col-sm-4 col-lg-2">
            <div className={`mx-auto mx-sm-0 ${style["title"]}
             ${
        ...
        {activeTitle === "accountInfo" && <AccountInfo user={user} token={token} />}
        {activeTitle === "changePassword" && <ChangePasswords user={user} token={token} />}
        {activeTitle === "myOrder" && <AccountOrders user={user}  token={token}/>}
        </div>
      </div>
    </div>
  );
